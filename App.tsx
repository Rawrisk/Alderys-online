  ...
  const performAITurn = useCallback(() => {
    ...
    const isCombatWinnable = (q: number, r: number, additionalPower: number = 0) => {
      ...
      if (enemyUnits.length > 0) {
        ...
        return myPower >= enemyPower * 1.2; // Demand 20% more power for confidence
      }

      // NOTE: Boss/Dungeon checks use the power actually present at the hex (myPower),
      // not the whole army's power, since only units physically on the hex fight. Kept
      // as pure power thresholds (no unit-composition gate) - a composition requirement
      // (e.g. "needs 2 mages") can stall the AI forever if it never reaches that exact
      // mix, which caused a real deadlock where the AI never approached the boss at all.
      if (isBoss) {
        return myPower >= 150;
      }
              ...
              if (possibleMoves.length > 0) {
                ...
                gameState.board.forEach(t => {
                  ...
                  if (t.monsterLevel) {
                    ...
                    // Composition bonus: prefer sending the unit type still missing
                    // from the stack forming here, so the AI builds a balanced strike
                    // force instead of piling up whichever unit is simply closest.
                    ...
                    if (t.monsterLevel === 1) {
                      ...
                      if (unit.type === 'warrior' && warriorsThere < 2) weight += 15;
                      ...
                      if (unit.type === 'mage' && magesThere === 0) weight += 15;
                      else if (unit.type === 'knight' && knightsThere === 0) weight += 15;
                    } else if (t.monsterLevel === 3) {
                      const magesThere = unitsThere.filter(u => u.type === 'mage').length;
                      const knightsThere = unitsThere.filter(u => u.type === 'knight').length;
                      if (unit.type === 'mage' && magesThere < 1) weight += 20;
                      else if (unit.type === 'knight' && knightsThere < 1) weight += 20;
                    }
                ...
                if (leadUnit && unit.id !== leadUnit.id) {
                  ...
                  myUnitsOnTargets.forEach(u => {
                    if (u.id !== unit.id) targets.push({ q: u.q, r: u.r, weight: 30 });
                  });
                }
